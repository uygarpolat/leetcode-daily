...
class Bank:
	...
	def transfer(self, account1: int, account2: int, money: int) -> bool:
		n = len(self.balance)
		if account1 > n-1 or account2 > n-1 or self.balance[account1] < money:
			return False
...
def main():
	...
	bank.withdraw(3, 10)
	assert bank.balance[3] == 10
	assert bank.transfer(5,1,20)
	assert bank.balance[5] == 10
	assert bank.balance[1] == 30
	assert bank.deposit(5,20)
	assert bank.balance[5] == 30
	assert not bank.transfer(3,4,15)
	assert bank.balance[3] == 10
	assert bank.balance[4] == 50
