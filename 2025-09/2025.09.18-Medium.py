...
class TaskManager:
    def __init__(self, tasks: List[List[int]]):
        ...
        for userId, taskId, priority in tasks:
            self.tally[taskId] = [priority, userId]
            self.h.append((-priority, -taskId))
    ...
    def edit(self, taskId: int, newPriority: int) -> None:
        self.tally[taskId][0] = newPriority
        heapq.heappush(self.h, (-newPriority, -taskId))

    def rmv(self, taskId: int) -> None:
        if taskId in self.tally:
            del self.tally[taskId]

    def execTop(self) -> int:
        
        while self.h:
            neg_p, taskId = heapq.heappop(self.h)
            taskId = -taskId
            if taskId not in self.tally:
                continue
            cur_p, userId = self.tally[taskId]
            if cur_p != -neg_p:
                continue
            del self.tally[taskId]
            return userId
