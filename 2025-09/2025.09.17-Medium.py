...
class FoodRatings:
	...
	def __init__(self, foods: List[str], cuisines: List[str], ratings: List[int]):
		...
		for f, c, r in zip(foods, cuisines, ratings):
			...
			self.curRating[f] = r
			self.tally[c].append((-r, f))

		for heap in self.tally.values():
			heapq.heapify(heap)

	def changeRating(self, food: str, newRating: int) -> None:
		self.curRating[food] = newRating
		c = self.food2cuisine[food]
		heapq.heappush(self.tally[c], (-newRating, food))

	def highestRated(self, cuisine: str) -> str:
		heap = self.tally[cuisine]
		while heap:
			neg_r, f = heap[0]
			if -neg_r == self.curRating[f]:
				return f
			heapq.heappop(heap)
