	...
	def mergeArrays(self, nums1: List[List[int]], nums2: List[List[int]]) -> List[List[int]]:
		...
		while pointer1 < len(nums1) or pointer2 < len(nums2):
			if pointer1 >= len(nums1):
				result.append([nums2[pointer2][0], nums2[pointer2][1]])
				pointer2 += 1
				continue
			if pointer2 >= len(nums2):
				result.append([nums1[pointer1][0], nums1[pointer1][1]])
				pointer1 += 1
				continue
			if nums1[pointer1][0] == nums2[pointer2][0]:
				result.append([nums1[pointer1][0], nums1[pointer1][1] + nums2[pointer2][1]])
				pointer1 += 1
				pointer2 += 1
			elif nums1[pointer1][0] < nums2[pointer2][0]:
				result.append([nums1[pointer1][0], nums1[pointer1][1]])
				pointer1 += 1
			else:
				result.append([nums2[pointer2][0], nums2[pointer2][1]])
